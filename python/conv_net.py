...
def conv_net(params, layers, data, labels, test=False):
    ...
    # print(f"Loss Layer Weight shape: {params[i-1]['w'].shape}")
    # print(f"Loss Layer Bias shape: {params[i-1]['b'].shape}")
    wb = np.concatenate([params[i-1]['w'].ravel(), params[i-1]['b'].ravel()])
    # print(f"Constructed wb size: {wb.size}")
...
def convnet_forward(params, layers, data, test=False):
    ...
    # print(f"Layer 0 (DATA layer) input shape: {data.shape}")

    for i in range(1, l-1):
        layer_type = layers[i]['type']

        # print(f"Layer {i} ({layer_type} layer):")
        # print(f"  Input shape: {output[i-1]['data'].shape}")
        ...
        # print(f"  Output shape: {output[-1]['data'].shape}")

    if test:
        # print(f"Layer {l-1} (LOSS layer):")
        # print(f"  Input shape: {output[l-2]['data'].shape}")

        W = params[l-2]['w'] @ output[l-2]['data'] + params[l-2]['b']
        # print(f"  After applying weights and biases, shape: {W.shape}")
