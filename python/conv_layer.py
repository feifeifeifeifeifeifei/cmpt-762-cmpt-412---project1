...
def conv_layer_forward(input_data, layer, param):
    ...
    w_col = param['w'].reshape(k * k * c, num)

    input_col_reshaped = input_col.reshape(k * k * c, -1)

    conv_out = np.dot(w_col.T, input_col_reshaped)

    conv_out = conv_out.reshape(num, h_out, w_out, batch_size)

    conv_out += param['b'].reshape(-1, 1, 1, 1)


    output['data'] = conv_out.reshape(h_out * w_out * num, batch_size)
